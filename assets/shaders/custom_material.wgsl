...
fn wobble(position: vec4<f32>, speed: f32, waves: f32, amplitude: f32) -> vec4<f32> {
    ...
    result.x += sin(position.y * waves + globals.time * speed) * amplitude;
    result.x += sin(position.z * waves + globals.time * speed) * amplitude;
...
fn fragment(mesh: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var noise = perlin_noise_3D(mesh, u32(6), globals.time);
